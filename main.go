...
func main() {
	rootCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			mainCopy(args[0], args[1], func() {
			}, func(p int, c string, s string, r string) {
				fmt.Fprint(os.Stdout, "\r\r\r\r")
				fmt.Printf("%d%s • %s/%s • %s remaining ", p, "%", c, s, r)
			}, func(err error) {
				if err != nil {
					log.Fatal(err)
				} else {
...
func mainCopy(
	...
	destFileDescriptor, err := os.OpenFile(destFilePath, os.O_RDWR|os.O_CREATE, srcFileInfo.Mode())
	if err != nil {
		onFinish(err)
		return
	}
